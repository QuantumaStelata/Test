...
def delet(id, message):
    ...
    if IsDelete:
        if not Values:
            const.bot.edit_message_text(chat_id=message.chat.id, message_id=const.copy[str(message.chat.id)]['ListID'] + 1, text='У тебя нет дел 😔')
        else:
            ...
            const.bot.edit_message_text(chat_id=message.chat.id, message_id=const.copy[str(message.chat.id)]['ListID'] + 1, text=new_text)
